...
contract("Oracles", async (accounts) => {
  ...
  it("can request flight status", async () => {
    // ARRANGE
    let flight = "Flight 1 1"; // Course number
    let timestamp = 1; // Math.floor(Date.now() / 1000);

    // Submit a request for oracles to get status information for a flight
    let response = await config.flightSuretyApp.fetchFlightStatus(
      config.firstAirline,
      flight,
      timestamp
    );
    ...
    console.log(
      "Fetch flight status index",
      response.logs[0].args.index.toNumber()
    );
    // ACT
    ...
    for (let a = 1; a < TEST_ORACLES_COUNT; a++) {
      ...
      for (let idx = 0; idx < 3; idx++) {
        try {
          ...
          console.log("OK", idx, oracleIndexes[idx].toNumber());
        } catch (e) {
